public class Instruction {
    ...
    public void executeADD(){
        ...
        temp2 = (secondRegisterData) & 0x000000FF,
        carry = ((temp1 + temp2) & (1 << 8)) == 1 << 8 ? 1 : 0,
        c1 = ((1 << 8) & (temp1+temp2)) == 0 ? 0 : 1,
        c2 = (((1 << 7) & (temp1+temp2))) == 0 ? 0 : 1,
        c3 = (((1 << 7) & (temp1))) == 0 ? 0 : 1,
        c4 = (((1 << 7) & (temp2))) == 0 ? 0 : 1,
        overflow = c1 ^ c2 ^ c3 ^ c4,
        negative = res < 0 ? 1 : 0,
        zero = res == 0 ? 1 : 0;

        setSREGByte(5, carry);
        setSREGByte( 4, overflow);
        setSREGByte(3, negative);
        setSREGByte(2, overflow ^ negative);
        setSREGByte(1, zero);
    ...
    public void executeBEQZ(){
        if (firstRegisterData == 0) {
            System.out.println("SECOND REG NUM IS "+secondRegisterNumber);
    ...
    public void setSREGByte(int byteNumber, int data){
        CPU.getCPU().getRegisters().setSREG((byte) (CPU.getCPU().getRegisters().readSREG() | data << (byteNumber - 1)));
    }
